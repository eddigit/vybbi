-- Fix ambiguous column reference in get_social_feed function
CREATE OR REPLACE FUNCTION public.get_social_feed(user_id_param uuid, limit_param integer DEFAULT 20, offset_param integer DEFAULT 0)
 RETURNS TABLE(id uuid, user_id uuid, profile_id uuid, content text, post_type text, visibility text, related_id uuid, created_at timestamp with time zone, author_display_name text, author_avatar_url text, author_profile_type profile_type, likes_count bigint, comments_count bigint, user_has_liked boolean, media_attachments jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
