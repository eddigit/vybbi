-- Create enum for view types
CREATE TYPE view_type AS ENUM ('full_profile', 'quick_preview', 'search_result');
...
CREATE TABLE public.profile_views (
  ...
  view_type view_type NOT NULL DEFAULT 'full_profile',
  referrer_page TEXT,
  session_id UUID NOT NULL DEFAULT gen_random_uuid(),
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  
  -- Index for performance
  CONSTRAINT unique_view_per_session UNIQUE (viewed_profile_id, viewer_user_id, session_id, created_at::date)
);
...
-- RLS Policies
-- Artists can view who visited their profile
...
CREATE POLICY "Users can track their visits" 
ON public.profile_views 
FOR INSERT 
...
CREATE OR REPLACE FUNCTION public.track_profile_view(
  ...
  p_view_type view_type DEFAULT 'full_profile',
...
BEGIN
  ...
  ON CONFLICT ON CONSTRAINT unique_view_per_session DO NOTHING
...
CREATE INDEX idx_profile_views_viewer_user ON public.profile_views(viewer_user_id);
CREATE INDEX idx_profile_views_created_at ON public.profile_views(created_at);
CREATE INDEX idx_profile_views_viewer_profile ON public.profile_views(viewer_profile_id);