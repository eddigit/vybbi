-- Check if slug column exists and create functions/triggers that might be missing
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_profiles_slug') THEN
    CREATE UNIQUE INDEX idx_profiles_slug ON public.profiles(slug) WHERE slug IS NOT NULL;
  END IF;
END $$;

-- Generate slugs for existing profiles that don't have them
