-- Create a secure RPC function for affiliate tracking
...
-- Create trigger function to automatically update clicks count when a visit is inserted
...
DROP TRIGGER IF EXISTS trigger_update_affiliate_clicks ON public.affiliate_visits;
CREATE TRIGGER trigger_update_affiliate_clicks
  AFTER INSERT ON public.affiliate_visits
  FOR EACH ROW
  EXECUTE FUNCTION public.update_affiliate_clicks_on_visit();

-- Allow public access to the tracking function
GRANT EXECUTE ON FUNCTION public.track_affiliate_visit TO anon, authenticated;

-- Allow anonymous users to insert into affiliate_visits (already covered by existing RLS)