...
export default function AgentProfile() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-foreground mb-4">Agent non trouvé</h1>
          <p className="text-muted-foreground">
            Cet agent n'existe pas ou n'est pas accessible publiquement.
          </p>
        </div>
      </div>
    );
  ...
  const description = profile.bio 
    ...
    : `Découvrez le profil de ${profile.display_name}, agent artistique ${profile.location ? `basé à ${profile.location}` : ''} sur Vybbi.`;

  return (
    <>
      <SEOHead
        title={`${profile.display_name} - Agent Artistique`}
