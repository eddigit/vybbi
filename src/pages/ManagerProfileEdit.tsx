...
export default function ManagerProfileEdit() {
  ...
  const fetchArtistRoster = async () => {
    try {
      ...
      if (data) {
        // Separate by status
        const confirmedIds = data.filter(ma => ma.representation_status === 'accepted').map(ma => ma.artist_profile_id);
        const pendingIds = data.filter(ma => ma.representation_status === 'pending').map(ma => ma.artist_profile_id);
  ...
  const fetchAvailableArtists = async () => {
    try {
      ...
      const existingIds = existingData?.map(ma => ma.artist_profile_id) || [];
  ...
  const handleAddArtist = async (artistId: string) => {
    try {
      const { error } = await supabase
        .from("manager_artists")
        .insert([{
          manager_profile_id: id,
          artist_profile_id: artistId,
          representation_status: 'pending'
        }]);

      if (error) throw error;
      toast.success("Management request sent to artist");
      fetchArtistRoster();
      fetchAvailableArtists();
    } catch (error) {
      console.error("Error sending management request:", error);
      toast.error("Failed to send management request");
    }
  };
    ...
    <div className="container mx-auto p-6 space-y-6">
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center space-x-4 mb-6">
            <Avatar className="h-20 w-20">
              ...
              <AvatarFallback className="text-lg">
                {profileData.display_name?.charAt(0)?.toUpperCase() || "M"}
              </AvatarFallback>
            </Avatar>
            <div>
          ...
          <div>
            ...
            <Input
              ...
              placeholder="Your name or management company name"
            />
          </div>
          ...
          <div>
            ...
            <Textarea
              ...
              placeholder="Tell us about your management services and experience"
              rows={4}
            />
          </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          {/* Add Artists */}
          <div>
            <h4 className="font-medium mb-4">Send Management Request</h4>
