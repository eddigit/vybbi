  ...
  useEffect(() => {
    const fetchAgent = async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('vybbi_agents').select('id').eq('user_id', user.id).single();
  ...
  const loadProspectingTemplates = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from('email_templates').select('*').or('type.like.%prospect%,type.like.%vybbi%').eq('is_active', true).order('name');
  ...
  const loadBrevoTemplates = async () => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.functions.invoke('brevo-templates');
      if (error) throw error;
        ...
        const {
          ...
          body: {
            ...
            to: [{
              ...
              name: selectedProspect.contact_name
            }],
            params: brevoParams,
  ...
  return <Dialog open={isOpen} onOpenChange={open => {
    if (!open) {
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        {selectedProspect && <Card className="mb-6">
            <CardHeader>
            ...
            <CardContent>
              <div className="flex flex-wrap gap-4">
                ...
                {selectedProspect.company_name && <div className="flex items-center gap-2">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="select" className="space-y-4">
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  ...
                  <Switch checked={brevoMode} onCheckedChange={enabled => {
            ...
            <div className="grid gap-4">
              ...
              {isLoading ? <div className="text-center py-8 flex items-center justify-center gap-2">
                  <Loader2 className="h-4 w-4 animate-spin" />
                  {brevoMode ? 'Chargement des templates Brevo...' : 'Chargement des templates...'}
                </div> : brevoMode ?
            // Brevo Templates
            brevoTemplates.length === 0 ? <div className="text-center py-8 text-muted-foreground">
                    <ExternalLink className="h-8 w-8 mx-auto mb-2 opacity-50" />
                    Aucun template Brevo trouvé
                  </div> : <div className="grid gap-3">
                    {brevoTemplates.map(template => <Card key={template.id} className={`cursor-pointer transition-all hover:shadow-md ${selectedBrevoTemplate?.id === template.id ? 'ring-2 ring-primary' : ''}`} onClick={() => handleBrevoTemplateSelect(template)}>
            ...
            // Internal Templates
            templates.length === 0 ? <div className="text-center py-8 text-muted-foreground">
                    Aucun template de prospection trouvé
                  </div> : <div className="grid gap-3">
                    {templates.map(template => <Card key={template.id} className={`cursor-pointer transition-all hover:shadow-md ${selectedTemplate?.id === template.id ? 'ring-2 ring-primary' : ''}`} onClick={() => handleTemplateSelect(template)}>
                        <CardHeader className="pb-3">
                          <div className="flex items-center justify-between">
          ...
          <TabsContent value="customize" className="space-y-4">
            {(selectedTemplate || selectedBrevoTemplate) && <div className="space-y-6">
                <Card>
                  <CardHeader>
                  ...
                  <CardContent className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <Input id="subject" value={customSubject} onChange={e => setCustomSubject(e.target.value)} placeholder="Sujet de l'email..." />
                      <p className="text-xs text-muted-foreground">
                        {brevoMode ? 'Variables Brevo disponibles selon votre template' : 'Les variables {{contact_name}}, {{company_name}} seront automatiquement remplacées'}
                      </p>
                    </div>
                ...
                <div className="space-y-4">
                        ...
                        <div className="grid gap-3">
                          {Object.entries(brevoParams).map(([key, value]) => <div key={key} className="space-y-1">
                              <Label htmlFor={key} className="text-sm capitalize">
                                {key.replace(/_/g, ' ')}
                              </Label>
                              <Input id={key} value={value} onChange={e => setBrevoParams(prev => ({
                        ...prev,
                        [key]: e.target.value
                      }))} placeholder={`Valeur pour ${key}`} />
                            </div>)}
                          <Button variant="outline" size="sm" onClick={() => setBrevoParams(prev => ({
                      ...prev,
                ...
                <div className="space-y-2">
                        ...
                        <Textarea id="custom-message" value={customMessage} onChange={e => setCustomMessage(e.target.value)} placeholder="Ajoutez un message personnel qui sera inséré dans le template..." rows={6} />
                        <p className="text-xs text-muted-foreground">
          ...
          <TabsContent value="preview" className="space-y-4">
            {(selectedTemplate || selectedBrevoTemplate) && <div className="space-y-4">
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                  ...
                  <CardContent>
                    {brevoMode ? <div className="space-y-4">
                        <div className="p-4 border rounded-lg bg-blue-50 dark:bg-blue-950/30">
                          ...
                          <div className="mt-3 space-y-2">
                            {Object.entries(brevoParams).map(([key, value]) => <div key={key} className="flex justify-between text-sm">
                      ...
                      </div> : <div className="border rounded-md p-4 bg-background max-h-96 overflow-y-auto" dangerouslySetInnerHTML={{
                ...
                <div className="flex gap-3 justify-end">
                  ...
                  <Button onClick={handleSendEmail} disabled={isSending} className="flex items-center gap-2">
                    {isSending ? <Loader2 className="h-4 w-4 animate-spin" /> : brevoMode ? <ExternalLink className="h-4 w-4" /> : <Send className="h-4 w-4" />}
